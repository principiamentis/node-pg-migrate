...
export function createPolicy(mOptions: MigrationOptions) {
  const _create: CreatePolicy = (tableName, policyName, options = {}) => {
    ...
    if (typeof options.restrictive === 'boolean')
      clauses.unshift(`AS ${options.restrictive ? 'RESTRICTIVE' : 'PERMISSIVE'}`)
    const clausesStr = clauses.join(' ')
    const policyNameStr = mOptions.literal(policyName)
    const tableNameStr = mOptions.literal(tableName)
    const stmts = [`CREATE POLICY ${policyNameStr} ON ${tableNameStr} ${clausesStr};`]
    if (options.comment) stmts.push(makeComment(`POLICY ${policyNameStr} ON`, tableNameStr, options.comment))
    return stmts.join('\n')
  }
...
export function alterPolicy(mOptions: MigrationOptions) {
  const _alter: AlterPolicy = (tableName, policyName, options = {}) => {
    ...
    const tableNameStr = mOptions.literal(tableName)
    const stmts = []
    if (clauses.length) stmts.push(`ALTER POLICY ${policyNameStr} ON ${tableNameStr} ${clauses.join(' ')};`)
    if (typeof options.comment !== 'undefined')
      stmts.push(makeComment(`POLICY ${policyNameStr} ON`, tableNameStr, options.comment))
    return stmts.join('\n')
  }
  return _alter
}
